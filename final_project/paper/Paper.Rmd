...
In a perfect world, the evolution of organisms could be just as easily retraced by the evolution of their *functions* as by their sequence; after all the selective pressure is on the mechinisms and activities of an organism and not on its DNA sequence. \marginnote{Although you may get lower resolution, because silent mutations or such that don't alter the function of a protein would be missed.}
...
We are currently investigating the reason for this strange phenomenon and the best explanatioi we've come up with so far is that *Brachy* and *Medicago* are not actually representative of their respective group; they have been chosen as model organisms because they are easy to sequence but that's mainly because their genome is much smaller and less complex than that of actual crop plants. *Medicago* is also well known for having an unusally high rate of genomic evolution, gene births and gene deaths [@Young2011].

\begin{marginfigure}
\includegraphics{resources/expectedTree.png}
\caption{Expected cladogram according to sequence based phylogenetics. \citep{Kellogg2001,Azani2017,Puttick2018}}
\label{fig:expectedTree}
\end{marginfigure}

It is difficult to come to a clear evolutionary tree within the same species (I am talking about maize^[Many of the maize lines have more than one ancestor and there is an immense amount of horizontal gene transfer with maize, even to closely related species.]) but it is encouraging to see that in the generated tree the 3 different classes of maize (stiff stalk, non-stiff stalk, and iodent) are differentiated from each other in a reasonable way.
...
So it seems the answer is not that simple.

## Tracing Back the Signal
Another big question is why does the tree actually look the way it does? Is it truly a biologically meaningful display of differences and similarities in function or is it just an artifactual tree, mainly caused by a bias in the method.
...
There might still be some biological significance to it but we're not sure yet what that might be. So for now it seems that some of the signal clearly comes from biological differences while some other part might be due to the method.
